...
export default function ContactPage({ params }: { params: { lang: 'en' | 'ar' } }) {
  return (
    <div className="space-y-10">
      ...
      />
      <form
        className="grid gap-4 rounded-2xl border border-[rgb(var(--border))] p-6"
        action="mailto:a.elsherif79@gmail.com"
        method="post"
        encType="text/plain"
      >
        <Input name="name" placeholder={params.lang === 'ar' ? 'الاسم الكامل' : 'Full name'} required />
        <Input name="email" type="email" placeholder={params.lang === 'ar' ? 'البريد الإلكتروني' : 'Email'} required />
        <Textarea
          name="message"
